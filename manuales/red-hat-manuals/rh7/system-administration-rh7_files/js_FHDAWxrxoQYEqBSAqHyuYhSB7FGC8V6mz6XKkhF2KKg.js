chrometwo_require(['jquery', 'moment', 'equalHeights', 'base', 'chrome_lib', 'drupal-admin-drawer'], function($, moment, equalHeights, base, lib, drawer) {
$(document).ready(function () {
    var adminDrawerTypes = ["page","product", "product2", "prima_product", "vulnerability"];
    if (adminDrawerTypes.indexOf(Drupal.settings.kcs.nodeType) > -1) {
        if ('nodeId' in Drupal.settings.kcs && Drupal.settings.kcs.nodeId.length > 0 && (window.location.pathname.indexOf("/edit") == -1)) {
            function fetch_drawer_data(user_info) {
                if(user_info.internal) {
                    $.ajax('/api/drawers/'+Drupal.settings.kcs.nodeId).done(drawer.init);
                }
            }
  ...
  if (momentDates.length > 0){
    ...
    momentDates.each(function() {
      ...
      if (momentDate.isValid()){
        ...
        var full_text = momentDate.format(momentDate.locale(locale).calendar('sameElse', momentDate));
